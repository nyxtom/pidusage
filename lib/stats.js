...
var stats = {
  ...
  proc_calc: function(pid, options, done) {
    ...
    if(history.fd) {
      return read(history.fd, compute)
    }

    fs.open(p.join('/proc', ''+pid, 'stat'), 'r', function(err, fd) {
      if(err)
        return done(err)

      history.fd = fd

      read(history.fd, compute)
    })

    /**
     * Compute /proc/{pid}/stat data to readable memory/cpu usage
     * @param {Error} err (from fs.read)
     * @param {Number} bytesRead
     * @param {Buffer} infos
     */
    function compute(err, bytesRead, infos) {
      if(err)
        return done(err, null)

      infos = infos.toString('utf8', 0, bytesRead)
      ...
      history.uptime = cpu.uptime

      self.history[pid] = history
    ...
    function read(fd, cb) {
      if(!history.buffer)
        history.buffer = new Buffer(kReadFileBufferLength)

      fs.read(fd, history.buffer, 0, kReadFileBufferLength, 0, cb)
    }
  },
  /**
